

const Education = () => {
    return (
        <div className="mt-2 space-y-2">
            <div>
                <p className="text-orange-400">BSC in CSE</p>
                <div className="text-sm">
                    <div className="flex">
                        <p className="font-medium w-16">Institute</p>
                        <p>: International University of Business Agriculture and Technology, Dhaka</p>
                    </div>
                    <div className="flex">
                        <p className="font-medium w-16">Session</p>
                        <p>: 2020 - 23</p>
                    </div>
                    <div className="flex">
                        <p className="font-medium w-16">CGPA</p>
                        <p>: 3.58 (out of 4.00)</p>
                    </div>
                </div>
            </div>

            <div>
                <p className="text-orange-400">Higher Secondary Certificate (HSC)</p>
                <div className="text-sm">
                    <div className="flex">
                        <p className="font-medium w-16">Institute</p>
                        <p>: Shashidal Alhaj Abu-Taher College, Cumilla</p>
                    </div>
                    <div className="flex">
                        <p className="font-medium w-16">Session</p>
                        <p>: 2016 - 17</p>
                    </div>
                    <div className="flex">
                        <p className="font-medium w-16">GPA</p>
                        <p>: 4.75 (out of 5.00)</p>
                    </div>
                </div>
            </div>

            <div>
                <p className="text-orange-400">Secondary School Certificate (SSC)</p>
                <div className="text-sm">
                    <div className="flex">
                        <p className="font-medium w-16">Institute</p>
                        <p>: Kasba Government High School, Brahmanbaria</p>
                    </div>
                    <div className="flex">
                        <p className="font-medium w-16">Session</p>
                        <p>: 2011 - 2015</p>
                    </div>
                    <div className="flex">
                        <p className="font-medium w-16">CGPA</p>
                        <p>: 4.50 (out of 5.00)</p>
                    </div>
                </div>
            </div>

        </div>
    );
};

export default Education;